...
Inorder to execute the R code for the Lower Grade Glioma (LGG) data set (a type of brain cancer), 
within the R environment execute:
>source("LGGdemo.R")



Joint subspace or Eigenvectors of the Joint Laplacian Lr* is written to file : LrStar_Eigenvectors.txt
...
The eigenvalues of the Joint Laplacian Lr* is written to file : LrStar_Eigenvalues.txt

k-means clustering can be performed on the rows of LrStar_Eigenvectors matrix to get the clusters in the data set.
The cluster assignments are written to the file LGG-ClusterAssignment.txt for the LGG data set.



File ConvexLaplacian.R contains the R implementation of the CoALa algorithm[1] as a function ConvexLaplacian. 
...
Default value: NULL.
if rank=NULL, the algorithm varies the rank between ceiling(K/M) to 50 and selects the optimal rank r* of the joint subspace.
...
#Log Transform of Sequence based RNA and miRNA modality
LogData=Data
